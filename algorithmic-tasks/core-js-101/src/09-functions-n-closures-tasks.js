...
/**
 * Returns the functions composition of two specified
 * functions f(x) and g(x).
 * The result of compose is to be a function of one argument,
 * (lets call the argument x),
 * which works like applying function f to the result of applying
 * function g to x, i.e.
...
function getPolynom(...coefficients) {
  return (x) => coefficients.reduce(
    (result, coeff, index) => result + coeff * x ** (coefficients.length - 1 - index),
    0,
  );
}
...
function logger(func, logFunc) {
  return (...args) => {
    const funcHeader = `${func.name}(${args.map(
      JSON.stringify,
    ).join(',')})`;
    logFunc(`${funcHeader} starts`);
    const result = func(...args);
    logFunc(`${funcHeader} ends`);
    return result;
  };
}
